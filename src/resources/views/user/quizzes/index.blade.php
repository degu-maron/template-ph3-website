        ...
        @foreach ($quizzes as $quiz)
            <div class="p-quiz-list__item">
                <div class="p-quiz-list__item__image-wrapper">
                    <img class="p-quiz-list__item__image" src="{{ $quiz->image }}" alt="クイズ{{ $loop->iteration }}" width="300" height="400">
                </div>
                <h2 class="p-quiz-list__item__title">{{ $quiz->name }}</h2>
                <p class="p-quiz-list__item__description">{{ $quiz->description }}</p>
                <a href="{{ route('quizzes.detail', ['quiz' => $quiz]) }}" class="p-quiz-list__item__button">
                    <x-primary-button class="bg-blue-700">クイズを解く</x-primary-button>
                </a>
            </div>
        @endforeach
    </div>
